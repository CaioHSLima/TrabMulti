...
```{r}
hep = scale(heptathlon)[,1:8] #Padronizando as variáveis para mesma escala (utilizar  var escore ou nao?)
p <- ncol(hep)
m.cor<-cor(hep)
...
round(aut.val,digits = 2)
100*aut.val/nvar
sum((100*aut.val/nvar)[1:2])
...
# Usando a função principal (só resultados iniciais)

```{r}
reprincomp <- principal(hep,nfactors=2,rotate="none")
```
...
```{r}
# Comunalidade
cbind(reprincomp$communality)
```

```{r}
# Especificidade
cbind(reprincomp$uniquenesses)
```

```{r}
# Proporção da Variabilidade explicada
reprincomp$Vaccounted
```

```{r}
# Matris de resíduos: R-LL'
reprincomp$residual
```

```{r}
# QMresidual associado a  matriz R-LL'
reprincomp$rms
```
#Classificação
...
```{r}
score_comp = reprincomp$loadings[,1]
...
h_new_score = hep_score %>% arrange(-escore) #%>% dplyr::select(escore,score,rankreal)
ranknovo = c(seq(1,25,1))
h_new_score = cbind(h_new_score,ranknovo)[,9:11] 
```
