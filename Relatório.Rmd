...
Foi estimado então os parâmetros do Modelo de Regressão Normal Linear Multivariada (MRNLM), dado por $\bf{Y} = \bf{X}\bf{B} + \bf{\xi}$, onde $\bf{Y}$ é a matriz de dados (supõe que $\bf{Y}$ segue uma normal 3-variada), $\bf{X}$ a matriz de planejamento, $\bf{B}$ a matriz parâmetros de regressão e $\xi$ a matriz de erros. Como visto na Tabela \ref{MRNLM}, todas as estimativas foram significativas. Dessa forma, é interessante verificar quais medidas se diferem entre os sexos, e portanto aplicamos um teste CBU. 
